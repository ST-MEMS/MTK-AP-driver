...
#define HTS_FUN(f)		 printk(HTS_TAG "%s\n", __func__);
...
static int hts221_set_powermode(struct i2c_client *client, enum HTS_POWERMODE_ENUM power_mode)
{
	...
	dat = !START_NEW_CONVERT;
	err = hts221_i2c_write_block(client, REG_CNTRL2_ADDR, &dat, 1);
	if (err < 0)
		return err;
...
static int hts221_hmdy_enable_nodata(int en)
{
	if (1 == en) {
		if (!obj_i2c_data->is_hmdy_enabled) {
			if (!obj_i2c_data->is_temp_enabled)
				hts221_enable_nodata(en);

			obj_i2c_data->is_hmdy_enabled = true;
		}
	}
	...
	if (0 == en) {
		if (obj_i2c_data->is_hmdy_enabled) {
			...
			obj_i2c_data->is_hmdy_enabled = false;
		}
	}
...
static int hts221_temp_enable_nodata(int en)
{
	if (1 == en) {
		if (!obj_i2c_data->is_temp_enabled) {
			if (!obj_i2c_data->is_hmdy_enabled) {
			...
			obj_i2c_data->is_temp_enabled = true;
		}
	}
	...
	if (0 == en) {
		if (obj_i2c_data->is_temp_enabled) {
			...
			obj_i2c_data->is_temp_enabled = false;
		}
	}
...
static int hts221_set_delay(u64 ns)
{
	...
	res = hts221_set_sampling_period(obj->client, odr, HTS221_BDU);
	if (res < 0) {
		HTS_ERR("Error hts221_set_sampling_period\n");
		return res;
	}
...
static int hts221_get_temp_data(int *value, int *status)
{
	char buff[HTS221_BUFSIZE];
	int err = 0;

	HTS_FUN();
	err = hts221_get_temperature(obj_i2c_data->client, buff, HTS221_BUFSIZE);
	if (err) {
		HTS_ERR("get compensated temperature value failed," "err = %d\n", err);
		return -1;
	}
...
static int hts221_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->is_hmdy_enabled = false;
	obj->is_temp_enabled = false;
	...
	ctl.enable_nodata = hts221_hmdy_enable_nodata;
...
static int hts221_temp_local_init(void)
{
	struct hts221_i2c_data *obj = obj_i2c_data;
	struct temp_control_path tctl = { 0 };
	struct temp_data_path tdata = { 0 };
	int err = 0;

	if (-1 == hts221_init_flag)
        return -1;

	tctl.is_use_common_factory = false;
	tctl.open_report_data = hts221_open_report_data;
	tctl.enable_nodata = hts221_temp_enable_nodata;
	tctl.set_delay = hts221_set_delay;
	tctl.is_report_input_direct = false;
	tctl.is_support_batch = obj->hw->is_batch_supported;
	err = temp_register_control_path(&tctl);
	if (err) {
		HTS_ERR("register temp control path err\n");
		return -2;
	}

	tdata.get_data = hts221_get_temp_data;
	tdata.vender_div = 1000;
	err = temp_register_data_path(&tdata);
	if (err) {
		HTS_ERR("temp_register_data_path failed, err = %d\n", err);
		return -3;
	}

	err = batch_register_support_info(ID_TEMPRERATURE, obj->hw->is_batch_supported, tdata.vender_div, 0);
	if (err) {
		HTS_ERR("register temp batch support err = %d\n", err);
		return -4;
	}
...
static int hts221_temp_remove(void)
{
	return 0;
}

static struct temp_init_info hts221_temp_init_info = {
	.name = "hts221_temp",
	.init = hts221_temp_local_init,
	.uninit = hts221_temp_remove,
};
...
MODULE_AUTHOR("hongxu.zhao@mediatek.com, William.ZENG");
