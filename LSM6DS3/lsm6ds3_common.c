...
int lsm6ds3_i2c_write_with_mask(struct i2c_client *client,u8 addr, u8 mask, u8 data)
{
	...
	res = lsm6ds3_i2c_read_block(client, addr, &old_data,1);
	if (res < 0)
		return res;
	...
	return lsm6ds3_i2c_write_block(client, addr, &new_data,1);
}

#if(CONFIG_STEP_COUNTER||CONFIG_STEP_DETECT)
int lsm6ds3_step_configure(struct lsm6ds3_data *obj)
{
	...
	res = lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_FUNC_CFG_ACCESS, LSM6DS3_REG_FUNC_CFG_ACCESS_MASK_FUNC_CFG_EN, LSM6DS3_EN); 
	if (res <0) {
		ST_ERR("write LSM6DS3_REG_FUNC_CFG_ACCESS register enable err!\n");
		return LSM6DS3_ERR_I2C;
	}

	databuf = 0x8e;
	//4g ,and threshould is 14*32mg=448mg
	res = lsm6ds3_i2c_write_block(client, LSM6DS3_REG_PEDO_THS, &databuf, 0x01); 
	if (res <0) {
		ST_ERR("write LSM6DS3 step threshold err!\n");
		return LSM6DS3_ERR_I2C;
	}

	databuf = 0x5f;
	//DEB_TIME = 0x0B(11*80ms=880ms), DEB_STEP = 0x07(7Step)
	res = lsm6ds3_i2c_write_block(client, LSM6DS3_REG_PEDO_DEB, &databuf, 0x01); 
	if (res <0) {
		ST_ERR("write LSM6DS3 step debounce  err!\n");
		return LSM6DS3_ERR_I2C;
	}

	res = lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_FUNC_CFG_ACCESS, LSM6DS3_REG_FUNC_CFG_ACCESS_MASK_FUNC_CFG_EN, LSM6DS3_DIS); //DIS embeded registers
	if (res <0) {
		ST_ERR("write LSM6DS3_REG_FUNC_CFG_ACCESS register disable err!\n");
		return LSM6DS3_ERR_I2C;
	}

#if( CONFIG_PEDOMETER_ALWAYS_ON) //enable pedometer 
	res = lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_CTRL1_XL, LSM6DS3_REG_CTRL1_XL_MASK_FS_XL, LSM6DS3_REG_CTRL1_XL_FS_4G);
	if (res < 0) {
		ST_ERR("write 4g fullscalee err!\n");
		return LSM6DS3_ERR_I2C;
	}

	res = lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_CTRL1_XL, LSM6DS3_REG_CTRL1_XL_MASK_ODR_XL, LSM6DS3_REG_CTRL1_XL_ODR_26HZ);
	if (res < 0) {
		ST_ERR("write 26hz odr err!\n");
		return LSM6DS3_ERR_I2C;
	}

	res = lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_CTRL10_C, LSM6DS3_FUNC_ENABLE_MASK, LSM6DS3_EN);
	if (res < 0) {
		ST_ERR("enble func err!\n");
		return LSM6DS3_ERR_I2C;
	}

	res = lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_TAP_CFG, LSM6DS3_STEP_ENABLE_MASK,LSM6DS3_EN );
	if (res < 0) {
...
static void lsm6ds3_irq_work_func(struct work_struct *work)
{
	struct lsm6ds3_data *obj = obj_i2c_data;
	struct i2c_client *client = obj->client;
	int res;
	...
	res = lsm6ds3_i2c_read_block(client, LSM6DS3_REG_FUNC_SRC, &buf, 0x01);
	if (res < 0) {
		goto work_exit;
	}

#if (CONFIG_SIGNIFICANT_MOTION)
	//if ((LSM6DS3_FlAG_SIGN_MOTION_MASK & buf) && obj->significant_enabled) {
	if ((LSM6DS3_FLAG_STEP_MASK & buf) && obj->significant_enabled) {
		counter++;
		if (counter > 6) {
			counter = 0;
	...
	if ((LSM6DS3_FLAG_STEP_MASK & buf) && obj->step_d_enabled) {
		step_notify(TYPE_STEP_DETECTOR);
	}
#endif

#if (CONFIG_TILT)
	if ((LSM6DS3_FLAG_TILT_MASK & buf) && obj->tilt_enabled) {	
		tilt_notify();
	}
#endif
...
int lsm6ds3_set_interrupt(void)
{
	...
	res = lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_CTRL3_C, LSM6DS3_INT_ACTIVE_MASK, LSM6DS3_EN);
	if (res < 0) {
	...
	if (node) {
		...
		if (request_irq(obj->irq, lsm6ds3_isr, IRQ_TYPE_LEVEL_LOW, "gyroscope", NULL)) {		
...
static ssize_t lsm6ds3_attr_i2c_show_reg_value(struct device_driver *ddri, char *buf)
{
	...
	res = lsm6ds3_i2c_read_block(obj->client, obj->reg_addr, &reg_value, 0x01);
	if (res < 0) {
...
int lsm6ds3_i2c_create_attr(struct device_driver *driver) 
{
	...
	for (idx = 0; idx < num; idx++) {
		if ((res = driver_create_file(driver, lsm6ds3_attr_i2c_list[idx]))) {            
			ST_ERR("driver_create_file (%s) = %d\n", lsm6ds3_attr_i2c_list[idx]->attr.name, res);
			break;
		}
	}   
...
static int lsm6ds3_suspend(struct i2c_client *client, pm_message_t msg)
{
	...
	if (msg.event == PM_EVENT_SUSPEND) {
		if (obj->acc_enabled == 1) {
			...
			res = lsm6ds3_acc_set_power_mode(acc_obj, false);
			if (res) {
		...
		if (obj->gyro_enabled == 1) {
			...
			mutex_lock(&lsm6ds3_op_mutex);
			res = lsm6ds3_gyro_set_power_mode(gyro_obj, false);
			if (res) {
...
static int lsm6ds3_resume(struct i2c_client *client)
{
	...
	if (obj->acc_enabled == 1) {
		...
		mutex_lock(&lsm6ds3_op_mutex);
		res = lsm6ds3_acc_set_power_mode(acc_obj, true);
		if (res) {
			mutex_unlock(&lsm6ds3_op_mutex);
	...
	if (obj->gyro_enabled == 1) {
		...
		mutex_lock(&lsm6ds3_op_mutex);
		res = lsm6ds3_gyro_set_power_mode(gyro_obj, true);
		if (res) {
			mutex_unlock(&lsm6ds3_op_mutex);
...
static int lsm6ds3_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	res = lsm6ds3_check_device_id(obj);
	if (res) {
