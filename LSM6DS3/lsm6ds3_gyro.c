...
static int lsm6ds3_gyro_set_resolution(struct lsm6ds3_gyro *gyro_obj)
{
	...
	res = lsm6ds3_i2c_read_block(client, LSM6DS3_REG_CTRL2_G, &dat, 0x01);
	if (res < 0) {
				...
				if (gyro_obj->fir.num < firlen) {                
					...
					if (atomic_read(&gyro_obj->trace) & ADX_TRC_FILTER) {
						ST_LOG("add [%2d] [%5d %5d %5d] => [%5d %5d %5d]\n", gyro_obj->fir.num,
							gyro_obj->fir.raw[gyro_obj->fir.num][LSM6DS3_AXIS_X], 
							gyro_obj->fir.raw[gyro_obj->fir.num][LSM6DS3_AXIS_Y], 
							gyro_obj->fir.raw[gyro_obj->fir.num][LSM6DS3_AXIS_Z],
							gyro_obj->fir.sum[LSM6DS3_AXIS_X], gyro_obj->fir.sum[LSM6DS3_AXIS_Y], 
							gyro_obj->fir.sum[LSM6DS3_AXIS_Z]);
					}
					gyro_obj->fir.num++;
					gyro_obj->fir.idx++;
				} else {
					...
					if (atomic_read(&gyro_obj->trace) & ADX_TRC_FILTER) {
						ST_LOG("add [%2d] [%5d %5d %5d] => [%5d %5d %5d] : [%5d %5d %5d]\n", idx,
							gyro_obj->fir.raw[idx][LSM6DS3_AXIS_X], 
							gyro_obj->fir.raw[idx][LSM6DS3_AXIS_Y], 
							gyro_obj->fir.raw[idx][LSM6DS3_AXIS_Z],
							gyro_obj->fir.sum[LSM6DS3_AXIS_X], 
							gyro_obj->fir.sum[LSM6DS3_AXIS_Y], 
							gyro_obj->fir.sum[LSM6DS3_AXIS_Z],
...
static int lsm6ds3_gyro_write_calibration(struct lsm6ds3_gyro *gyro_obj, int dat[LSM6DS3_AXES_NUM])
{
	...
	if (!gyro_obj || !dat) {
		...
		gyro_obj->cali_sw[LSM6DS3_AXIS_X] += gyro_obj->cvt.sign[LSM6DS3_AXIS_X]*cali[gyro_obj->cvt.map[LSM6DS3_AXIS_X]];
		gyro_obj->cali_sw[LSM6DS3_AXIS_Y] += gyro_obj->cvt.sign[LSM6DS3_AXIS_Y]*cali[gyro_obj->cvt.map[LSM6DS3_AXIS_Y]];
		gyro_obj->cali_sw[LSM6DS3_AXIS_Z] += gyro_obj->cvt.sign[LSM6DS3_AXIS_Z]*cali[gyro_obj->cvt.map[LSM6DS3_AXIS_Z]];
	} 
...
static int lsm6ds3_gyro_set_full_scale(struct lsm6ds3_gyro *gyro_obj, u8 dataformat)
{
	...
	res = lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_CTRL2_G, LSM6DS3_REG_CTRL2_G_MASK_FS_G, dataformat);
	if (res < 0) {
...
static int lsm6ds3_gyro_init(struct lsm6ds3_gyro *gyro_obj, bool enable)
{
	...
	res = lsm6ds3_gyro_set_full_scale(gyro_obj, LSM6DS3_REG_CTRL2_G_FS_2000DPS);
	if (res < 0) {
	...
	gyro_obj->odr = 0;
	res = lsm6ds3_gyro_set_power_mode(gyro_obj, enable);
	if (res < 0) {
	    	ST_ERR("lsm6ds3_gyro_init step 4!\n");
	    	return res;
	}
...
static int lsm6ds3_gyro_read_data(struct lsm6ds3_gyro *gyro_obj, u8 *data, int bufsize)
{
	...
	if (gyro_obj->lsm6ds3_gyro_power == false) {
		lsm6ds3_gyro_set_power_mode(gyro_obj, true);
...
static ssize_t lsm6ds3_attr_gyro_show_firlen_value(struct device_driver *ddri, char *buf)
{
	...
	if (atomic_read(&gyro_obj->firlen)) {
		...
		for (idx = 0; idx < len; idx++) {
			ST_LOG("[%5d %5d %5d]\n", gyro_obj->fir.raw[idx][LSM6DS3_AXIS_X], gyro_obj->fir.raw[idx][LSM6DS3_AXIS_Y], 
				gyro_obj->fir.raw[idx][LSM6DS3_AXIS_Z]);
		}

		ST_LOG("sum = [%5d %5d %5d]\n", gyro_obj->fir.sum[LSM6DS3_AXIS_X], gyro_obj->fir.sum[LSM6DS3_AXIS_Y], 
			gyro_obj->fir.sum[LSM6DS3_AXIS_Z]);
		ST_LOG("avg = [%5d %5d %5d]\n", gyro_obj->fir.sum[LSM6DS3_AXIS_X]/len, gyro_obj->fir.sum[LSM6DS3_AXIS_Y]/len, 
			gyro_obj->fir.sum[LSM6DS3_AXIS_Z]/len);
	}
...
{    
	...
	if (gyro_obj->lsm6ds3_gyro_hw) {
		len += snprintf(buf+len, PAGE_SIZE-len, "CUST: i2c_num=%d, direction=%d, sensitivity = %d,(power_id=%d,\
				power_vol=%d)\n", gyro_obj->lsm6ds3_gyro_hw->i2c_num, gyro_obj->lsm6ds3_gyro_hw->direction, 
				gyro_obj->reso->sensitivity, gyro_obj->lsm6ds3_gyro_hw->power_id, 
				gyro_obj->lsm6ds3_gyro_hw->power_vol);   
		dumpReg(obj);
	} else {
...
static ssize_t lsm6ds3_attr_gyro_store_chipinit_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	lsm6ds3_gyro_init(gyro_obj, false);
	dumpReg(obj);
...
int lsm6ds3_gyro_create_attr(struct device_driver *driver) 
{
	...
	for (idx = 0; idx < num; idx++) {
		if ((res = driver_create_file(driver, lsm6ds3_attr_gyro_list[idx]))) {            
			ST_ERR("driver_create_file (%s) = %d\n", lsm6ds3_attr_gyro_list[idx]->attr.name, res);
			break;
		}
	}    
...
static int lsm6ds3_gyro_enable_nodata_intf(int en)
{
	...
	ST_LOG("lsm6ds3_gyro_enable_nodata_intf OK![%d]\n", en);
	return LSM6DS3_SUCCESS;
}
...
static int lsm6ds3_gyro_set_delay_intf(u64 ns)
{
	...
	if (value <= 2) {
		...
		sample_delay = LSM6DS3_REG_CTRL2_G_ODR_26HZ;
	}
...
static long lsm6ds3_gyro_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case GYROSCOPE_IOCTL_SET_CALI:
			...
			if (copy_from_user(&sensor_data, data, sizeof(sensor_data))) {
				...
				res = lsm6ds3_gyro_write_calibration(gyro_obj, cali);
			}
			break;
		...
		case GYROSCOPE_IOCTL_GET_CALI:
			...
			res = lsm6ds3_gyro_read_calibration(gyro_obj, cali);
			if (res) {
				break;
			}
...
#ifdef CONFIG_COMPAT
static long lsm6ds3_gyro_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    long res = 0;
...
static int lsm6ds3_gyro_local_init(struct platform_device *pdev)
{
	...
	lsm6ds3_gyro_reset_calibration(gyro_obj);

	for (retry = 0; retry < 3; retry++) {
		res = lsm6ds3_gyro_init(gyro_obj, false);
		if (res) {
