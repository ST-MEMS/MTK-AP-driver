...
static int LSM6DS3_embeded_feature_enable(int sensor ,int enable)
{
	...
	if (enable) {
		enable_bit = LSM6DS3_EN;
	} else {
		enable_bit = LSM6DS3_DIS;
	}
	...
	if (((obj->tilt_enabled+obj->significant_enabled+obj->step_d_enabled+obj->step_c_enabled)==0x01)&&enable_bit) {
		if ((lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_CTRL10_C, LSM6DS3_FUNC_ENABLE_MASK, LSM6DS3_EN))<0) {
			ST_ERR("write LSM6DS3_REG_CTRL10_C  func enable err!\n");
			return LSM6DS3_ERR_I2C;
		}

		if (!obj->acc_enabled) {
			if ((lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_CTRL1_XL, LSM6DS3_REG_CTRL1_XL_MASK_ODR_XL, 
			     LSM6DS3_REG_CTRL1_XL_ODR_26HZ)) < 0) {
				ST_ERR("write LSM6DS3_REG_CTRL1_XL  ODR enable err!\n");
				return LSM6DS3_ERR_I2C;
			}	
		}
	}
	...
	if (!(obj->tilt_enabled||obj->significant_enabled||obj->step_d_enabled||obj->step_c_enabled)) {
		if ((lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_CTRL10_C, LSM6DS3_FUNC_ENABLE_MASK, LSM6DS3_DIS)) < 0) {
			ST_ERR("write LSM6DS3_REG_CTRL10_C  func dis err!\n");
			return LSM6DS3_ERR_I2C;
		}

		if (!obj->acc_enabled) {
			if ((lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_CTRL1_XL, LSM6DS3_REG_CTRL1_XL_MASK_ODR_XL, 
			     LSM6DS3_REG_CTRL1_XL_ODR_0HZ)) < 0) {
				ST_ERR("write LSM6DS3_REG_CTRL1_XL  ODR dis err!\n");
				return LSM6DS3_ERR_I2C;
			}		
		}
	}
	...
	if ((sensor==ST_SENSOR_STEP_COUNTER)||(sensor==ST_SENSOR_STEP_DETECT)||(sensor==ST_SENSOR_SIGNIFICANT_MOTION)) {
		if ((((obj->step_d_enabled+obj->step_c_enabled+obj->significant_enabled)==1)&enable_bit)||
			...
			if ((lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_TAP_CFG, LSM6DS3_STEP_ENABLE_MASK,enable_bit )) < 0) {
				ST_ERR("write LSM6DS3_REG_TAP_CFG step enable err!\n");
				return LSM6DS3_ERR_I2C;
			} 
			ST_LOG("LSM6DS3 enable step ok  en=%d\n",enable_bit);
		}
	}
#endif
	...
	if ((sensor==ST_SENSOR_STEP_DETECT)||(sensor==ST_SENSOR_SIGNIFICANT_MOTION)) {
		if ((((obj->significant_enabled+obj->step_d_enabled)==0x01)&&enable_bit) ||
			...
			if ((lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_INT1_CTRL, LSM6DS3_STEP_INT_MASK, enable_bit)) < 0) {
				ST_ERR("write LSM6DS3_REG_INT1_CTRL step int enable err!\n");
				return LSM6DS3_ERR_I2C;
			} 
			ST_LOG("LSM6DS3 enable step d int ok en=%d\n",enable_bit);
		}
	}


	if (sensor == ST_SENSOR_SIGNIFICANT_MOTION) {
		if ((lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_CTRL10_C, LSM6DS3_SIGN_MOTION_ENABLE_MASK, enable_bit)) < 0) {
			ST_ERR("write LSM6DS3_REG_CTRL10_C sig_motion enable err!\n");
			return LSM6DS3_ERR_I2C;
		}

		if ((lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_INT1_CTRL, LSM6DS3_SIGN_MOTION_INT_MASK, enable_bit)) < 0) {
			ST_ERR("write LSM6DS3_REG_INT1_CTRL sign_motion int enable err!\n");
			return LSM6DS3_ERR_I2C;
		} 
	}

	if (sensor == ST_SENSOR_TILT) {
		if ((lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_TAP_CFG, LSM6DS3_TILT_ENABLE_MASK, enable_bit)) < 0) {
			ST_ERR("write  LSM6DS3_REG_TAP_CFG tilt enable  err!\n");
			return LSM6DS3_ERR_I2C;
		}

		if ((lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_MD1_CFG, LSM6DS3_TILT_INT_MASK, enable_bit)) < 0) {
			ST_ERR("write LSM6DS3_REG_MD1_CFG tilt int enable err!\n");
			return LSM6DS3_ERR_I2C;
		} 
	}
...
//==============tilt================================
#if(CONFIG_TILT)
static int lsm6ds3_tilt_open_report_data_intf(int en)
{
	struct lsm6ds3_data *obj = obj_i2c_data;
	int res = 0;

	obj->tilt_enabled = en;
	res = LSM6DS3_embeded_feature_enable(ST_SENSOR_TILT, en);
...
static int lsm6ds3_tilt_set_delay_intf(uint64_t delay)
{
	int res = 0;
	return res;
}
...
static int lsm6ds3_tilt_get_data_intf(int *value, int *status)
{
	ST_FUN(); 
#if(!CONFIG_HARDWARE_INTERRUPT)
	tilt_notify();
...
static int lsm6ds3_tilt_local_init(void)
{
	...
	tilt_ctl.open_report_data= lsm6ds3_tilt_open_report_data_intf;	
	tilt_ctl.set_delay=lsm6ds3_tilt_set_delay_intf;
	...
	tilt_data.get_data = lsm6ds3_tilt_get_data_intf;
...
static int lsm6ds3_tilt_local_remove(void)
{
	ST_FUN();
	return LSM6DS3_SUCCESS;
}

struct tilt_init_info  lsm6ds3_tilt_init_info = {
	.name   = "LSM6DS3_TILT",
	.init   = lsm6ds3_tilt_local_init,
	.uninit = lsm6ds3_tilt_local_remove,
};
#endif
...
static int lsm6ds3_pedo_reset_counter(struct lsm6ds3_pedo *pedo_obj)
{
	...
	res = lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_CTRL10_C, LSM6DS3_REG_CTRL10_C_MASK_PEDO_RST_STEP,LSM6DS3_EN);
	if (res < 0) {
...
{
	return LSM6DS3_SUCCESS; //lsm6ds3_pedo_reset_counter(pedo_obj);
}
...
static int lsm6ds3_pedo_read_data(struct lsm6ds3_pedo *pedo_obj, u32 *value)
{
	...
	res = lsm6ds3_i2c_read_block(client, LSM6DS3_REG_STEP_COUNTER_L, (u8*)&counter, 0x02);
	if (res < 0) {
		...
		if (clear) {
			res = lsm6ds3_i2c_write_with_mask(client, LSM6DS3_REG_CTRL10_C, LSM6DS3_REG_CTRL10_C_MASK_PEDO_RST_STEP,LSM6DS3_DIS);
			if (res < 0) {
...
static int lsm6ds3_step_counter_open_report_data_intf(int open)
{
	return LSM6DS3_SUCCESS;
}

static int lsm6ds3_step_counter_enable_nodata_intf(int en)
{
	struct lsm6ds3_data *obj = obj_i2c_data;
	int res = 0;

	obj->step_c_enabled = en;
	res = LSM6DS3_embeded_feature_enable(ST_SENSOR_STEP_COUNTER, en);    
	if (res) {
		ST_ERR("lsm6ds3_step_counter_enable fail!\n");
		return res;
	}
	ST_LOG("lsm6ds3_step_counter_enable_nodata_intf OK! en=%d\n", en);
	return LSM6DS3_SUCCESS;
}

#if(CONFIG_SIGNIFICANT_MOTION||CONFIG_STEP_COUNTER)
static int lsm6ds3_enable_sigificant_intf(int en)
{
	struct lsm6ds3_data *obj = obj_i2c_data;
	int res = 0;

	obj->significant_enabled = en;
	res = LSM6DS3_embeded_feature_enable(ST_SENSOR_SIGNIFICANT_MOTION, en);    
	if (res) {
		ST_ERR("lsm6ds3_significant_enable fail!\n");
		return res;
	}

	ST_LOG("lsm6ds3_significant_enable_intf OK! en=%d\n",en);
	return LSM6DS3_SUCCESS;
}
#endif

#if(CONFIG_STEP_DETECT||CONFIG_STEP_COUNTER)
static int lsm6ds3_enable_step_dectect_intf(int en)
{
	struct lsm6ds3_data *obj = obj_i2c_data;
	int res =0;

	obj->step_d_enabled = en;
	res = LSM6DS3_embeded_feature_enable(ST_SENSOR_STEP_DETECT, en);    
	if (res) {
		ST_ERR("lsm6ds3_step_detect_enable fail!\n");
		return res;
	}

	ST_LOG("lsm6ds3_step_detect_enable_intf OK! en=%d \n", en);
	return 0;
}
#endif

#if(CONFIG_STEP_DETECT||CONFIG_STEP_COUNTER)
static int lsm6ds3_step_d_set_delay_intf(u64 ns)
{
	return LSM6DS3_SUCCESS;
}
#endif

static int lsm6ds3_step_c_set_delay_intf(u64 ns)
{
	struct lsm6ds3_data *obj = obj_i2c_data;
	struct lsm6ds3_pedo *pedo_obj = &obj->lsm6ds3_pedo_data;
	int value = 0;
	...
	if (value <= 5) {
		sample_delay = LSM6DS3_REG_CTRL1_XL_ODR_208HZ;
	} else if (value <= 10) {
		sample_delay = LSM6DS3_REG_CTRL1_XL_ODR_104HZ;
	} else {
		sample_delay = LSM6DS3_REG_CTRL1_XL_ODR_52HZ;
	}
	...
	ST_LOG("lsm6ds3_pedo_set_delay_intf (%d)\n",value);
	return LSM6DS3_SUCCESS;
}

#if(CONFIG_SIGNIFICANT_MOTION||CONFIG_STEP_COUNTER)
static int lsm6ds3_get_data_significant_intf(uint32_t *value, int *status)
{
#if(!CONFIG_HARDWARE_INTERRUPT)
...
#if(CONFIG_STEP_DETECT||CONFIG_STEP_COUNTER)
static int lsm6ds3_get_data_step_d_intf(uint32_t *value, int *status)
{
#if(!CONFIG_HARDWARE_INTERRUPT)
...
static int lsm6ds3_get_data_step_c_intf(uint32_t *value, int *status)
{
	struct lsm6ds3_data *obj = obj_i2c_data;
	struct lsm6ds3_pedo *pedo_obj = &obj->lsm6ds3_pedo_data;
	int res = 0;
...
static int lsm6ds3_pedo_local_init(void)
{
	...
	if ((res = lsm6ds3_pedo_create_attr(&(lsm6ds3_pedo_init_info.platform_diver_addr->driver)))) {
		ST_ERR("create attribute err = %d\n", res);
		goto exit_create_attr_failed;
	}

	for (retry = 0; retry < 3; retry++) {
		res = lsm6ds3_pedo_init(pedo_obj, 1);
		if (res == 0) {
			break;
		} else {
	...
	ctl.open_report_data 	   = lsm6ds3_step_counter_open_report_data_intf;
	ctl.enable_nodata 	   = lsm6ds3_step_counter_enable_nodata_intf;
	ctl.enable_step_detect     = lsm6ds3_enable_step_dectect_intf;
	ctl.enable_significant     = lsm6ds3_enable_sigificant_intf;
	...
	data.get_data 		  = lsm6ds3_get_data_step_c_intf;
	data.vender_div 	  = 1;
	data.get_data_step_d      = lsm6ds3_get_data_step_d_intf;
	data.get_data_significant = lsm6ds3_get_data_significant_intf;
