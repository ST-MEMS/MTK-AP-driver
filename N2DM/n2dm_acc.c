            ...
            if(atomic_read(&acc_obj->fir_en) && !atomic_read(&acc_obj->suspend))
            {
                ...
                {                
                    ...
                    if(atomic_read(&acc_obj->trace) & ADX_TRC_FILTER)
                    {
                        ST_LOG("add [%2d] [%5d %5d %5d] => [%5d %5d %5d]\n", acc_obj->fir.num,
                            acc_obj->fir.raw[acc_obj->fir.num][N2DM_AXIS_X], acc_obj->fir.raw[acc_obj->fir.num][N2DM_AXIS_Y], acc_obj->fir.raw[acc_obj->fir.num][N2DM_AXIS_Z],
                            acc_obj->fir.sum[N2DM_AXIS_X], acc_obj->fir.sum[N2DM_AXIS_Y], acc_obj->fir.sum[N2DM_AXIS_Z]);
                    }
                    acc_obj->fir.num++;
                    acc_obj->fir.idx++;
                }
                else
                {
                    ...
                    if(atomic_read(&acc_obj->trace) & ADX_TRC_FILTER)
                    {
                        ...
                        acc_obj->fir.raw[idx][N2DM_AXIS_X], acc_obj->fir.raw[idx][N2DM_AXIS_Y], acc_obj->fir.raw[idx][N2DM_AXIS_Z],
                        acc_obj->fir.sum[N2DM_AXIS_X], acc_obj->fir.sum[N2DM_AXIS_Y], acc_obj->fir.sum[N2DM_AXIS_Z],
                        data[N2DM_AXIS_X], data[N2DM_AXIS_Y], data[N2DM_AXIS_Z]);
                    }
                }
...
#endif
    if((res = n2dm_acc_read_rawdata(acc_obj, acc_obj->data)))
    {        
...
static int n2dm_acc_read_rawdata_string(struct n2dm_acc *acc_obj, u8 *buf)
{
    ...
    {
        sprintf(buf, "%04x %04x %04x", acc_obj->data[N2DM_AXIS_X], 
...
{
    struct n2dm_data *obj = obj_i2c_data;
    u8 strbuf[N2DM_BUFSIZE] = "unkown chipid";
    if(NULL == obj->client)
    {
        ST_ERR("i2c client is null!!\n");
        return 0;
...
static ssize_t n2dm_attr_acc_show_cali_value(struct device_driver *ddri, char *buf)
{
    ...
    if((err = n2dm_acc_read_calibration(acc_obj, tmp)))
    {
        return -EINVAL;
    }
    else
    {    
        ...
        len += snprintf(buf+len, PAGE_SIZE-len, "[SW ][%d] (%+3d, %+3d, %+3d)\n", 1, 
            acc_obj->cali_sw[N2DM_AXIS_X], acc_obj->cali_sw[N2DM_AXIS_Y], acc_obj->cali_sw[N2DM_AXIS_Z]);
...
{ 
    ...
    {
        if((err = n2dm_acc_reset_calibration(acc_obj)))
        {
    ...
    else if(3 == sscanf(buf, "0x%02X 0x%02X 0x%02X", &x, &y, &z))
    {
        ...
        dat[N2DM_AXIS_Y] = y;
        dat[N2DM_AXIS_Z] = z;
        if((err = n2dm_acc_write_calibration(acc_obj, dat)))
        {
...
static ssize_t n2dm_attr_acc_show_firlen_value(struct device_driver *ddri, char *buf)
{
    ...
    if(atomic_read(&acc_obj->firlen))
    {
        ...
        ST_LOG("sum = [%5d %5d %5d]\n", acc_obj->fir.sum[N2DM_AXIS_X], acc_obj->fir.sum[N2DM_AXIS_Y], acc_obj->fir.sum[N2DM_AXIS_Z]);
        ST_LOG("avg = [%5d %5d %5d]\n", acc_obj->fir.sum[N2DM_AXIS_X]/len, acc_obj->fir.sum[N2DM_AXIS_Y]/len, acc_obj->fir.sum[N2DM_AXIS_Z]/len);
    }
...
{    
    ...
    if(acc_obj->n2dm_acc_hw)
    {
        len += snprintf(buf+len, PAGE_SIZE-len, "CUST: i2c_num=%d, direction=%d, sensitivity = %d,(power_id=%d, power_vol=%d)\n", 
                acc_obj->n2dm_acc_hw->i2c_num, acc_obj->n2dm_acc_hw->direction, acc_obj->reso->sensitivity, acc_obj->n2dm_acc_hw->power_id, acc_obj->n2dm_acc_hw->power_vol);   
        dumpReg(obj);
    }
    else
    {
...
/*----------------------------------------------------------------------------*/
static ssize_t n2dm_attr_acc_store_chipinit_value(struct device_driver *ddri, const char *buf, size_t count)
{
    struct n2dm_data *obj = obj_i2c_data;
    struct n2dm_acc *acc_obj = &obj->n2dm_acc_data;
...
/*----------------------------------------------------------------------------*/
static ssize_t n2dm_attr_acc_store_layout_value(struct device_driver *ddri, const char *buf, size_t count)
{
    struct n2dm_data *obj = obj_i2c_data;
    struct n2dm_acc *acc_obj = &obj->n2dm_acc_data;
...
static DRIVER_ATTR(cali,       S_IWUSR | S_IRUGO, n2dm_attr_acc_show_cali_value,          n2dm_attr_acc_store_cali_value);
static DRIVER_ATTR(power,                S_IRUGO, n2dm_attr_acc_show_power_status,        NULL);
static DRIVER_ATTR(firlen,     S_IWUSR | S_IRUGO, n2dm_attr_acc_show_firlen_value,        n2dm_attr_acc_store_firlen_value);
static DRIVER_ATTR(trace,      S_IWUSR | S_IRUGO, n2dm_attr_acc_show_trace_value,         n2dm_attr_acc_store_trace_value);
static DRIVER_ATTR(status,               S_IRUGO, n2dm_attr_acc_show_status_value,        NULL);
static DRIVER_ATTR(chipinit,   S_IWUSR | S_IRUGO, n2dm_attr_acc_show_chipinit_value,      n2dm_attr_acc_store_chipinit_value);
static DRIVER_ATTR(layout,     S_IRUGO | S_IWUSR, n2dm_attr_acc_show_layout_value,        n2dm_attr_acc_store_layout_value);
/*----------------------------------------------------------------------------*/
...
{
    int idx, err = 0;
    int num = (int)(sizeof(n2dm_attr_acc_list)/sizeof(n2dm_attr_acc_list[0]));
    if (driver == NULL)
    {
        return -EINVAL;
    }
    ...
    {
        if((err = driver_create_file(driver, n2dm_attr_acc_list[idx])))
        {            
...

{
    ...
    switch(cmd)
    {
        ...
        case GSENSOR_IOCTL_GET_CALI:
            ...
            if((err = n2dm_acc_read_calibration(acc_obj, cali)))
            {
                break;
            }
