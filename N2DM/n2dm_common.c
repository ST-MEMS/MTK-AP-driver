...
static DRIVER_ATTR(reg_value, S_IWUSR | S_IRUGO, n2dm_attr_i2c_show_reg_value, n2dm_attr_i2c_store_reg_value);
static DRIVER_ATTR(reg_addr,  S_IWUSR | S_IRUGO, n2dm_attr_acc_show_reg_addr,  n2dm_attr_i2c_store_reg_addr);
...
{
    int idx, err = 0;
    int num = (int)(sizeof(n2dm_attr_i2c_list)/sizeof(n2dm_attr_i2c_list[0]));
    if (driver == NULL)
    {
        return -EINVAL;
    }
    ...
    {
        if((err = driver_create_file(driver, n2dm_attr_i2c_list[idx])))
        {            
